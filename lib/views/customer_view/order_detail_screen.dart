...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  Future<void> _updateOrder(String status) async {
    ...
    if (response['status'] == "success") {
      ...
      // Khi giao thành công: chuyển 90% cho seller, 10% cho admin
      if (status == 'delivered' && widget.order.cartList.isNotEmpty) {
        final sellerId = widget.order.cartList.first.sellID;
        final total = widget.order.total;
        await userVM.addBalance(sellerId, total * 0.9);
        await adminVM.addBalance(total * 0.1);
      }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
          ...
          body:
              Consumer<OrderViewModel>(builder: (context, orderProvider, child) {
                        ...
                        children: [
                          ...
                          ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              CartItem item = widget.order.cartList[index] as CartItem;
